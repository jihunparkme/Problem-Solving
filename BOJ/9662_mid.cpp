...
int main(void)
{
	...
	// 6. (M-start+1) 에 사이클이 총 몇 번 나오는지 확인
	ll cntCycle = (M - start + 1) / cycleSize;

	// 7. 사이클 뒤에 있는 1 의 개수
	ll restCycle = 0;
	for (ll i = start + (cntCycle * cycleSize); i <= M; i++)
		if (winner[i % cycleSize]) restCycle++;

	// 8. 사이클 앞에 있는 1의 개수
	for (int i = 1; i < start; i++)
		if (winner[i]) restCycle++;
...
::중간에서 찾는 경우. a[idx + 1] ~ a[idx + n], a[idx + n + 1] ~ a[idx + 2n]
=> 패턴이 처음부터 시작된다는 보장이 없으니, 처음 패턴이 시작되는 위치를 찾고, 그 시점부터 끝까지 사이클 수를 구함
=> 이후 사이클 밖 1을 구한다.(앞은 1~start 까지, 뒤는 start + (cntCycle * cycleSize) ~ M)
